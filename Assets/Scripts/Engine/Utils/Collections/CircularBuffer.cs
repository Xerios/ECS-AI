    ...
    public class CircularBuffer<T>: IEnumerable<T>
    {
        ...
        public int IndexOf (Func<T, bool> predicate)
        {
            for (int i = 0; i < _size; i++) {
                if (predicate.Invoke(_buffer[i])) return i;
            }
            return -1;
        }
        ...
        {
            for (int i = 0; i < _size; i++) {
                if (predicate.Invoke(_buffer[i])) return true;
            }
            return false;
        }
        ...
        public void ForEach<U>(ActionRef<T, U> action, U extra)
        {
            for (int i = 0; i < _size; i++) {
                action.Invoke(ref _buffer[i], extra);
            }
        }
