...
namespace Engine
{
    public struct ObjectNode : IEquatable<ObjectNode>
    {
        public Entity entity;
        public Int3 position;

        public ObjectNode(Entity entity, Int3 position)
        {
            this.entity = entity;
    ...
    public class PointKDTree
    {
        ...
        readonly List<ObjectNode> largeList = new List<ObjectNode>();
        readonly Stack<ObjectNode[]> arrayCache = new Stack<ObjectNode[]>();
        static readonly IComparer<ObjectNode>[] comparers = new IComparer<ObjectNode>[] { new CompareX(), new CompareY(), new CompareZ() };
        ...
        class CompareX : IComparer<ObjectNode>
        {
            public int Compare (ObjectNode lhs, ObjectNode rhs) { return lhs.position.x.CompareTo(rhs.position.x); }
        }

        class CompareY : IComparer<ObjectNode>
        {
            public int Compare (ObjectNode lhs, ObjectNode rhs) { return lhs.position.y.CompareTo(rhs.position.y); }
        }

        class CompareZ : IComparer<ObjectNode>
        {
            public int Compare (ObjectNode lhs, ObjectNode rhs) { return lhs.position.z.CompareTo(rhs.position.z); }
        }
        ...
        public void Rebuild (List<ObjectNode> nodes)
        {
            for (int i = 0; i < tree.Length; i++) {
                var data = tree[i].data;
                if (data != null) {
                    for (int j = 0; j < LeafArraySize; j++) data[j] = default(ObjectNode);
                    arrayCache.Push(data);
                    tree[i].data = null;
                }
            }
            Build(1, nodes, 0, nodes.Count);
        }

        /** Rebuild the tree starting with all nodes in the array between index start (inclusive) and end (exclusive) */
        public void Rebuild (ObjectNode[] nodes, int start, int end)
        {
            ...
            numNodes = end - start;
            Build(1, new List<ObjectNode>(nodes), start, end);
        }

        ObjectNode[] GetOrCreateList ()
        {
            // Note, the lists will never become larger than this initial capacity, so possibly they should be replaced by arrays
            return arrayCache.Count > 0 ? arrayCache.Pop() : new ObjectNode[LeafArraySize];
        }
        ...
        void CollectAndClear (int index, List<ObjectNode> buffer)
        {
        ...
        void Build (int index, List<ObjectNode> nodes, int start, int end)
        {
            EnsureSize(index);
        ...
        void Add (ObjectNode point, int index, int depth = 0)
        {
        ...
        public ObjectNode GetNearest (Int3 point, Func<ObjectNode, bool> constraint)
        {
            ObjectNode best = default(ObjectNode);
        ...
        void GetNearestInternal (int index, Int3 point, Func<ObjectNode, bool> constraint, ref ObjectNode best, ref long bestSqrDist)
        {
            ...
            if (data != null) {
                for (int i = tree[index].count - 1; i >= 0; i--) {
                    ...
                    if (dist < bestSqrDist && (constraint == null || constraint(data[i]))) {
                        bestSqrDist = dist;
                        best = data[i];
                    }
                }
         ...
         */
        public void GetInRange (Int3 point, float sqrRadius, List<ObjectNode> buffer, int limit)
        {
            GetInRangeInternal(1, point, (long)((sqrRadius * Int3.FloatPrecision) * (sqrRadius * Int3.FloatPrecision)), buffer, limit);
        }

        void GetInRangeInternal (int index, Int3 point, long sqrRadius, List<ObjectNode> buffer, int limit)
        {
            if (buffer.Count == limit) return;
            var data = tree[index].data;
            ...
            if (data != null) {
                ...
                var childIndex = 2 * index + (dist < 0 ? 0 : 1);
                GetInRangeInternal(childIndex, point, sqrRadius, buffer, limit);

                // Try the other one if it is possible to find a valid node on the other side
                if (dist * dist < sqrRadius) {
                    // childIndex ^ 1 will flip the last bit, so if childIndex is odd, then childIndex ^ 1 will be even
                    GetInRangeInternal(childIndex ^ 0x1, point, sqrRadius, buffer, limit);
                }
            }
        }
    }
